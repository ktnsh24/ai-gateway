...
- 🫏 **Donkey:** Two parallel sets of stable blueprints — one for the AWS depot, one for the Azure hub — drawn so closely that any new wing on the AWS side has a matching wing on the Azure side.
...
| `variables.tf` | Inputs the operator overrides per environment | Adjustment dials at the top of the blueprint — region, environment tag, image to deploy, monthly hay budget. |
...
| Per-resource files | One concern per file (`ecs.tf`, `rds.tf`, …) so reviewers see only what changed | One blueprint sheet per stable wing, kept separate so a renovation in the kitchen doesn't churn the stables wing. |
...
| Resource | Type | Sizing | Purpose | 🫏 Donkey |
|----------|------|--------|---------|-----------|
| `aws_ecs_cluster.gateway` | ECS cluster | Container Insights enabled | Hosts the Fargate task running the gateway image | Front-yard plot where the dispatch shed actually stands and the donkeys start their shifts. |
...
| `aws_elasticache_cluster.redis` | ElastiCache | `cache.t3.micro`, single node, redis7 | Powers semantic cache + rate limiter | The pigeon-hole shelf and the trip-quota tally board, both stored in fast in-memory cubbies. |
| `aws_db_instance.postgres` | RDS PostgreSQL 16 | `db.t3.micro`, 20 GB, private | Stores the cost-tracking row log | The leather-bound expense ledger kept in a locked back office, written to on every donkey trip. |
| `aws_security_group.gateway` | SG | Inbound `0.0.0.0/0:8100`, all egress | Front-door firewall on the gateway container | Front-door bouncer rules — open the dispatch window to the world, let donkeys roam to any far stable. |
...
| `aws_iam_role.ecs_task` | IAM role | Inline `bedrock:InvokeModel*` policy | Lets the running container call AWS Bedrock | Donkey's saddlebag pass — the only badge that actually grants access to the AWS Bedrock far stable. |
...
| `aws_budgets_budget.cost_limit` | Budget | EUR limit (default `5`), tag-filtered | Triggers SNS at 80% + 100% of the cap | Stable owner's monthly hay-budget alarm — quietly chimes at 80%, screams at 100%. |
| `aws_lambda_function.budget_killer` | Lambda | Python 3.11, 128 MB, 5-minute timeout | Subscribed to the SNS topic; tears down ECS, RDS, ElastiCache, ECR, S3 when triggered | Emergency stable hand on a pager — once paged, walks the corridors and unplugs every wing the project owns. |
...
| Resource | Type | Sizing | Purpose | 🫏 Donkey |
|----------|------|--------|---------|-----------|
| `azurerm_resource_group.gateway` | Resource group | `rg-${prefix}` | Container for every other Azure resource | Plot of land the Azure stable is built on — every wing must sit inside this fenced area. |
...
| `azurerm_redis_cache.gateway` | Azure Cache for Redis | Basic C0, TLS 1.2 | Powers semantic cache + rate limiter | The Azure-side pigeon-hole shelf and trip-quota board, served over an encrypted internal cable. |
...
| `azurerm_consumption_budget_resource_group` *(in `budget.tf`)* | Consumption budget | EUR limit (default `5`) | Watches the resource group's spend | Azure-side hay-budget alarm scoped to this single fenced plot. |
| `azurerm_automation_account.budget_killer` | Automation account | Basic SKU | Hosts the kill-switch runbook | Pager on the wall outside the office — wakes the emergency stable hand when the budget alarm fires. |
...
| Layer | Mechanism | Effective behaviour | 🫏 Donkey |
|-------|-----------|---------------------|-----------|
| Sizing | `cache.t3.micro`, `db.t3.micro`, `B_Standard_B1ms`, 0.5 vCPU container | Smallest paid SKUs available on each cloud — €1–3/day at idle | Smallest stalls you can rent at each depot — enough room for a single donkey, no luxury pasture. |
...
| Budget alert | AWS Budget + Azure Consumption Budget, default cap **EUR 5** | SNS / Action Group fires at 80% and 100% | Owner's wall-clock alarm — chimes early at 80% so the stable hand can investigate calmly. |
| Kill switch | AWS Lambda + Azure Automation runbook subscribed to the 100% alert | Tears down ECS / Container App, RDS / Postgres, ElastiCache / Redis, ECR | Emergency stable hand who unplugs every wing of the project the moment the 100% alarm screams. |
...
| Signal | How it surfaces drift | 🫏 Donkey |
|--------|----------------------|-----------|
...
| Budget killer outputs | The Lambda/runbook log every resource it deleted; replaying after a kill confirms which wings actually existed | The emergency stable hand keeps a written list of every wing they unplugged for the morning post-mortem. |
...
`infra/` is the **set of stable blueprints**, not the stable itself. Two complete blueprint folders sit side by side — one labelled "AWS depot", one labelled "Azure hub" — and either can be raised, demolished, or rebuilt independently with a single `terraform apply` / `terraform destroy`.

Every wing is drawn for the same reason on both clouds: a yard for the dispatch shed (ECS cluster / Container Apps environment), a hired dispatch crew (Fargate task / Container App), a pigeon-hole shelf (ElastiCache / Azure Redis), an expense ledger in a locked office (RDS / Flexible Server), a uniform locker (ECR / ACR), keys for the right far stable (IAM roles / managed identity), and a log shed.

Glued onto both blueprints is an **owner-side cost guard**: a hay-budget alarm at €5/month that pages an emergency stable hand. The hand's runbook is brutal — when the 100% alarm screams, every wing the project owns is unplugged. That guarantee is what keeps the lab safe to leave running while you study other repos: nothing can quietly overshoot the monthly hay budget.
