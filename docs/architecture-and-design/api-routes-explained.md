...
> - [Architecture Overview](architecture.md) — how the routes sit inside the gateway
...
| Method | Path | Auth | Purpose | Deep dive | 🚚 Courier |
|--------|------|------|---------|-----------|-----------|
| `POST` | `/v1/chat/completions` | Bearer | Run a chat completion through cache → LLM → ledger | [completions-endpoint-explained.md](api-routes/completions-endpoint-explained.md) | The main delivery window — slip in, delivery if the pickup locker misses, line written in the expense ledger. |
...
| `GET` | `/v1/models` | Bearer | List the chat + embedding models on duty | [models-endpoint-explained.md](api-routes/models-endpoint-explained.md) | The roster pinned to the dispatcher's wall — every courier on shift and what parcel they carry. |
...
| `GET` | `/health` | **None** (public) | Combined liveness + readiness probe | [health-endpoint-explained.md](api-routes/health-endpoint-explained.md) | The front-porch lamp — watchman glances at it without knocking, sees the lights on the dispatch desk, pickup locker, and ledger. |
...
- 🚚 **Courier:** Two checks run on every shipping manifest before any window opens it (timing and API-key auth); each window then reaches into the same shared shelf of gateway tools to do its job.
