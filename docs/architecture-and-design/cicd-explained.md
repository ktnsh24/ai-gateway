...
| Workflow | Trigger | Stages | 🫏 Donkey |
|----------|---------|--------|-----------|
| `ci.yml` | `push` to `main`/`develop`, `pull_request` to `main` | Lint → Test → Docker Build | Robot stable hand who runs the report card on every shift change before letting anyone ride. |
| `deploy-aws.yml` | `workflow_dispatch` (manual, env = `dev` or `stg`) | OIDC login → ECR build & push → `terraform apply` → ECS service update | Same robot hand on the AWS depot side — only acts when an operator pushes the deploy button. |
...
| Job | Image / runtime | Steps | What it asserts | 🫏 Donkey |
|-----|-----------------|-------|-----------------|-----------|
...
| Stage | AWS workflow | Azure workflow | 🫏 Donkey |
|-------|--------------|----------------|-----------|
...
| Secret | Where stored | Used by | 🫏 Donkey |
|--------|--------------|---------|-----------|
...
| Branch | What runs | What deploys | 🫏 Donkey |
|--------|-----------|--------------|-----------|
...
| Failure | Symptom in CI | Recovery | 🫏 Donkey |
|---------|---------------|----------|-----------|
...
| Test regression | `test` job red on PR | Reproduce locally with `poetry run pytest -v`; the in-memory backends mean no Docker is needed | Dry-run shift exposed a misbehaving donkey; rehearse in the empty stable until it passes. |
...
| Budget killer fired between deploys | Workflow may target a deleted resource | Re-run the deploy workflow — Terraform recreates everything; expect a few minutes of cold start | Emergency stable hand demolished the wing; robot hand re-applies the blueprint and the dispatch desk is back. |
