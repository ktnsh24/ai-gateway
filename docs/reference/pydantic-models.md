...
|-------|----------|-----------|
| API specification | [API Contract](../architecture-and-design/api-contract.md) | 🚚 The API contract is the gateway's official rulebook specifying every shipping manifests format and response structure that clients must follow at the front door. |
| Architecture | [Architecture](../architecture-and-design/architecture.md) | 🚚 The architecture blueprint shows how the dispatch desk, pickup locker shelf, and expense ledger connect to route each courier delivery end-to-end across providers. |
| Config settings | [Getting Started](../setup-and-tooling/getting-started.md) | 🚚 The getting-started config guide sets up the gateway's environment variables so the switchboard knows which courier environment and pickup locker shelf to connect to. |
