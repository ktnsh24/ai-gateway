...
|---|----------|-------------------|-----------|
| 1 | [README.md](../README.md) | Project overview, features, tech stack, quick start | The stable's switchboard notice — what this dispatch desk does, which donkeys it can call, and how to flip the lights on. |
| 2 | [Gateway Concepts](ai-engineering/gateway-concepts.md) | What is an LLM gateway? Routing, caching, rate limiting, cost tracking — the 4 pillars explained | The training manual for the dispatcher — why every trip goes through one desk instead of each customer flagging down their own donkey. |
| 3 | [Architecture Overview](architecture-and-design/architecture.md) | System diagram, component relationships, request lifecycle | The dispatch-desk floor plan — every door, every shelf, every donkey stall, and the route a delivery slip travels through them. |
...
| # | Document | What you'll learn | 🫏 Donkey |
|---|----------|-------------------|-----------|
| 4 | [Getting Started](setup-and-tooling/getting-started.md) | Install Python, Poetry, env vars, first request | Open the stable, hire the staff, set up the front-door sign — first delivery slip goes through in five minutes. |
| 5 | [Docker Compose Guide](setup-and-tooling/docker-compose-guide.md) | Run the full stack (gateway + Redis + PostgreSQL) in containers | Portable mini-stable kit — pop the lid, the pigeon-hole shelf and ledger book are already wired in. |
...
| # | Document | What you'll learn | 🫏 Donkey |
|---|----------|-------------------|-----------|
| 7 | [LiteLLM Deep Dive](ai-engineering/litellm-deep-dive.md) | The universal harness — one OpenAI-format call, 100+ providers | The universal harness that fits any donkey — same reins, whether the donkey lives in AWS, Azure, or the local barn. |
| 8 | [Caching Deep Dive](ai-engineering/caching-deep-dive.md) | Semantic cache (cosine similarity in Redis), TTL, hit-rate tuning | The pigeon-hole of pre-written replies — if a near-identical question came in before, hand back the same note instantly, no donkey trip. |
| 9 | [Rate Limiting Deep Dive](ai-engineering/rate-limiting-deep-dive.md) | Fixed-window rate limit per API key, Redis vs in-memory | Trip quota per courier — each API key gets N trips per minute; on the (N+1)th the gate slams shut until the clock ticks over. |
| 10 | [Cost Tracking Deep Dive](ai-engineering/cost-tracking-deep-dive.md) | Per-request cost log, PostgreSQL schema, per-provider rollups | The leather-bound expense ledger — every donkey trip recorded with provider, tokens, and price, ready for the monthly invoice. |
| 11 | [Observability Deep Dive](ai-engineering/observability-deep-dive.md) | Structured logging, request IDs, latency histograms | The stable CCTV plus the tachograph on every donkey — every step of every trip is timestamped, tagged, and replayable. |
...
| 27 | [Phase 1 — Foundation Labs](hands-on-labs/hands-on-labs-phase-1.md) | First end-to-end gateway runs: cache hit rate, rate-limit behaviour, fallback under failure | First solo dispatch shifts — verify the pigeon-hole shelf hits, the gate slams at the quota, the backup donkey kicks in. |
| 28 | [Phase 2 — Production Labs](hands-on-labs/hands-on-labs-phase-2.md) | Multi-provider routing, cost-optimised strategy under load, observability dashboards | Busy-day dispatch — three donkeys on rotation, cheapest-first routing, watching the CCTV while it all runs. |
